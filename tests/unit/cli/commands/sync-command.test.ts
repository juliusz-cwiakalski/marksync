// tests/unit/cli/commands/sync-command.test.ts
//
// `syncCommand` branch coverage via the in-process `runCli` harness (GH-23).
// Mirrors plan-command.test.ts across sync's identical early-return paths
// (config/lock/creds/no-default/computePlan); sync's applyPlan success branch
// is unreachable without a live target and intentionally left uncovered.
...
/**
 * Env vars read transitively by `syncCommand` (credentials + the git/branch
...
describe("syncCommand — runCli branch coverage (--json)", () => {
	let dir: string;
	let origCwd: string;
	let envSnap: Record<string, string | undefined>;

	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "ms-sync-"));
		origCwd = process.cwd();
		envSnap = {};
	...
	async function runSyncJson(): Promise<{
		exit: number;
		parsed: Record<string, unknown>;
	}> {
		const s = newStreams();
		const exit = await runCli(["sync", "--json"], {
			stdout: s.stdout_w,
			stderr: s.stderr_w,
		});
		return {
	...
	test("1. no marksync.yml → INVALID_CONFIG, exit 10", async () => {
		const { exit, parsed } = await runSyncJson();
		expect(exit).toBe(10);
