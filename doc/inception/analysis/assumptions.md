...
| A-VAL-2 | The canonical GFM subset is sufficient for the target teams' real documentation (not a painful limitation). | value | testing | Adversarial public corpus vs design-partner docs | JC | MVP | Spike K1 (27/27 GFM); spec §9.5 subset |
...
| A-USA-1 | A new user can reach first publish in **<10 min** (excluding Atlassian credential creation). | usability | unvalidated | Moderated usability test (MLP) | JC | MLP | North star; OST E2.1; premortem `§3.3` |
| A-USA-2 | Setup friction (auth, base URL, space/page IDs, scopes) is the **primary** adoption barrier, not capability gaps. | usability | unvalidated | Support-issue taxonomy from beta | JC | MLP | Premortem `§4.2` |
...
| A-FEA-6 | Atlassian API drift (deprecations, rate limits, permission asymmetry) is **manageable** behind adapter boundaries. | feasibility | unvalidated | Adapter isolation + monitoring; re-run spike on API changes | JC | Ongoing | Premortem `§7.3`, `§7.4` |
...
- **A-FEA-1 (in-process Mermaid)** — first: it is load-bearing for the TypeScript choice (ADR-0001) and the single-binary promise. If invalidated, the implementation language is revisited. The ADR-0002 spike is the gating experiment.
- **A-VAL-1 (trust wedge differentiation)** — first value assumption: the entire strategy rests on safety/fidelity beating raw conversion breadth. Validate via beta before expanding scope.
- **A-FEA-2 (Bun single binary + signing)** — first feasibility-of-distribution: adoption and enterprise trust both depend on it. Clean-OS smoke before MVP release.
- **A-USA-1 (<10-min first publish)** — gates the MLP; if unmet, the wedge never activates regardless of capability.
- **A-VAL-3 (existing-corpus adoption)** — determines whether migration tooling must be in MLP rather than later; surfaced by red-team H4.
