...
| `INV-` | Invariant / release-blocking guardrail | `INV-<AREA>-<N>` | `INV-SAFE-1`: no silent overwrite; `INV-SAFE-2`: no silent re-create of REMOTE_DELETED; `INV-SAFE-3`: duplicate-UUID fatal; `INV-SEC-1`: no secrets in output |
