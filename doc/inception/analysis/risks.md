...
|---|---|---|---|---|---|---|---|
| R-VAL-1 | **Scope explosion** — delivering the end-state category vision before the wedge is retained (premortem #1). | value | High | High | Hard `MS-0002` / MVP beachhead contract + phase gates (Gate 1–6); every expansion passes an explicit gate; Decision Filter #6/#7. | JC | Medium |
| R-VAL-2 | **No validated beachhead** — building for an archetype, not 3–5 real retained teams (premortem #2 / FMEA #1, RPN 100). | value | High | High | **Recruit ≥3 design partners before `MS-0002` feature-lock; if 0 at feature-lock, slip the lock** (premortem `§18.1`); gate `MS-0003` on retention, not stars. | JC | Medium |
...
| R-USA-1 | **Setup friction kills activation** — auth/URL/space/page-ID/scope complexity loses users before the wedge is evaluated (premortem #3.3, #4.2). | usability | High | High | `MS-0003` / MLP milestone: guided `init`, `doctor` diagnostics, progressive disclosure, ≤ 10-min first-publish target; instrument TTV + setup-failure taxonomy during `MS-0002` beta. | JC | Medium |
| R-USA-2 | **No migration path** — adopting an existing Confluence corpus is hard, so MarkSync is "another publisher" (premortem #8). | usability | Medium | High | Track A-VAL-3; if confirmed, make migration a first-class `MS-0003` feature (premortem `§18.2`). | JC | Medium |
| R-USA-3 | **Safety mechanisms become permanent blockers** — one unsupported macro/stale lock/moved page blocks a whole subtree (premortem #4.3). | usability | Medium | Medium | Per-document isolation; **minimal `MS-0002` repair surface** (`repair-state` for stale locks + interrupted-apply journal replay, premortem `§14`); `explain`/`adopt`/`rebind`/`accept-remote`; distinguish warnings from hard blocks; deterministic conflict bundles. | JC | Low |
| R-FEA-1 | **In-process Mermaid unproven** — the official library may not render deterministically without Chromium, weakening the TypeScript rationale (premortem #6.4). | feasibility | Medium | High | ADR-0002 spike is the gating experiment; `MS-0002` `code`-fallback if the spike fails late (full render → `MS-0003`). **If the spike fails, impact is a language-level reconsideration and a multi-month `MS-0002` slip**, not merely a mitigation tweak. | JC | Medium |
...
| R-FEA-4 | **Plan accurate, apply not** — no cross-page transaction; partial apply leaves inconsistent state (premortem #4.4). | feasibility | Medium | Medium | No cross-page atomicity promised; preflight validation; optimistic concurrency per mutation; durable apply journal + **`MS-0002` `repair-state`/journal replay**; idempotent retry; explicit partial-apply status. | JC | Low |
...
| R-FEA-10 | **Permission asymmetry / partial visibility** — a token reads pages but not properties, or sees part of a tree; drift reasoning mis-classifies a page as "new" (duplicate) or "orphan" (premortem `§7.4`). If a page ID is recorded in the lock but returns `403 Forbidden`, MarkSync should assume the page exists but is inaccessible (emit a warning and skip) rather than treating it as deleted. Distinguishing deleted vs inaccessible requires assuming sync is performed by a space owner / power user with full read access. | feasibility | Medium | High | `doctor` capability discovery; visibility-completeness check before any create/adoption decision; block destructive decisions when visibility is incomplete; `403` on a locked page ID → warn + skip (not delete + recreate). | JC | Medium |
...
| R-VIA-1 | **Maintainer overload** — one maintainer owns a combinatorial matrix and burns out (premortem #7, #9.1, #9.3). | viability | High | High | Deliberately narrow `MS-0002` matrix (Cloud only; one auth path); **beachhead-critical vs validation-apparatus tagging** (apparatus may slip); contributor-sized seams; defer Data Center/OAuth/package managers. | JC | Medium |
