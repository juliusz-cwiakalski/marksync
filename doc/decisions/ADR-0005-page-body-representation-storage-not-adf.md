...
- **C-1 (lossless fidelity):** 27 of 27 golden `.md`/`.storage.xhtml` fixture pairs byte-match their snapshots (`tests/golden/fixtures/markdown/`), including a kitchensink fixture that round-trips all construct families in one body. The set now also pins front-matter stripping — the `marksync.uuid` block is dropped before rendering (`remark-frontmatter`, GH-63).
