...
- **2026-07-14 (GH-76, lifecycle phase 7 reconciliation)** — Fixed non-deterministic Mermaid SVG rendering and additive lock-merge (GH-69 regression):
  - **Kroki path now hashes normalized SVG bytes** (DEC-3): `Artifact.bytes` is the SVG normalized per §3.3 rules (strip comments, sort attributes, rewrite IDs, canonicalize whitespace, strip metadata), and `hash = sha256(normalizedSVG)`. This ensures deterministic digests across syncs even if Kroki emits non-deterministic IDs or ordering.
  - **Mermaid config passed as Kroki diagram options** (F-1): `deterministicIds` → `deterministic-ids=true`, `htmlLabels` → `html-labels=false` as query parameters. `securityLevel` is NOT passed (blocked by Kroki, enforced as `strict` by default, DEC-1).
  - **Lock file pruning** (F-3): `finalizeSuccessfulUpdate` now uses replacement semantics (`attachmentHashes = assetUploadHashes`) instead of additive merge, preventing indefinite accumulation of stale entries. `NO_CHANGE` outcomes preserve existing hashes (DEC-4).
  - **§3.3 normalization rules implemented**: `src/domain/mermaid/normalize.ts` lifts the full rule set from the GH-11 spike (`spikes/mermaid-render/normalize.ts`), pure and reusable for the in-process renderer (MS-0003+).
  - Restores NFR-PERF-4 idempotent-rerun invariant broken by GH-69: second sync with unchanged Mermaid content → 0 `uploadAttachment` calls, `NO_CHANGE` classification. `links.related_changes` extended with `GH-76`.
