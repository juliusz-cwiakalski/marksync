...
- **F-4 (Asset naming, DEC-1).** `src/domain/assets/naming.ts` exports a domain helper that produces `marksync-asset-<hash>.<ext>` from `{ hash, mime }` — **identical** to the existing infra `attachmentFilename()` for all user assets. The resolver uses this helper to compute the filename it writes into the HAST, and the infra `AttachmentService` independently derives the same filename from the `Artifact` at upload time — the two must agree byte-for-byte for ALL formats (an invariant test asserts it). SVG uses the `marksync-mermaid-` prefix in the infra **only when** `artifact.kind === "mermaid"` (reserved for E4-S1, gated on an explicit marker, not mime alone); user-authored `.svg` images use `marksync-asset-` like all other formats. (GH-26 review iter-1: gating on mime alone was a release-blocking bug; the fix adds an optional `kind` field to `Artifact` and gates the mermaid prefix on `kind === "mermaid"`, ensuring user SVG filenames match between HAST and upload.)
...
| DM-6 | Asset naming helper (NEW, domain/assets/naming.ts) | `assetFilename({ hash, mime }): string` → `marksync-asset-<hash>.<ext>`. Must agree with infra `attachmentFilename()` for ALL user asset formats (invariant test). Mermaid artifacts (future) use `kind="mermaid"` → `marksync-mermaid-` prefix (gated on explicit marker, not mime). |
...
- **AC-8 (naming-agreement invariant, DEC-1/NFR-10):** The domain `assetFilename({ hash, mime })` and the infra `attachmentFilename(artifact)` produce identical strings for ALL user asset formats (including SVG). Mermaid artifacts (future) use `kind="mermaid"` to gate the `marksync-mermaid-` prefix.
