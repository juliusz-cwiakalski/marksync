...
| DEC-5 | Use the full §3.3 normalization rule set for the Kroki path (OQ-1 resolved) | Rules are renderer-agnostic (operate on SVG bytes), provide defense-in-depth, are proven from the GH-11 spike, and avoid divergence with the future in-process renderer (MS-0003+). | 2026-07-14 |
...
| AC-F2-2 | **Given** a Mermaid diagram rendered via Kroki, **when** the normalized SVG is compared structurally to the raw Kroki SVG, **then** there are **0** structural differences in visual elements (normalization strips only non-deterministic metadata/IDs; all visual elements — paths, text, shapes — are preserved). Structural comparison via XML diff; pixel-level visual comparison is not available (happy-dom has no SVG layout engine per GH-11). | F-2, NFR-6 |
