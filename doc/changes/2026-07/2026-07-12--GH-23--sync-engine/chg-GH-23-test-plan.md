...
- Integration tests: `applyPlan` against `Bun.serve()` mock target (create/update/no-op/move/conflict/forbidden), crash→replay, lock+property update, idempotency, per-document isolation.
...
| AC-F2-1 | Duplicate-UUID corpus → `computePlan` returns `err(DuplicateUuid)` before any write (INV-SAFE-3) | TC-UNIT-001 | To Implement |
...
| AC-F10-1 | No credential/token in any output path (journal, plan, version.message, apply report) | TC-UNIT-005, TC-UNIT-006 | To Implement |
...
| DM-8 | Error-model (no change) | All TCs reuse existing `MarkSyncError` arms (no new arms) |
...
| NFR-3 (NFR-REL-8) | Duplicate-UUID fatal before any write | TC-UNIT-001 |
...
| NFR-8 (NFR-SEC-1) | No secrets in output: 0 credential/token in any output path | TC-UNIT-005, TC-UNIT-006 |
...
- **Coverage pattern**:
  - `apply-plan-integration.test.ts` — `applyPlan` against `Bun.serve()` mock target (create/update/no-op/move/conflict/forbidden).
  ...
  - `shell-git-safety-fuzz.test.ts` — malicious path/ref fuzz → rejected with 0 shell-execution surfaces.
- **Mocking**:
...
| TC-INTEGRATION-009 | Malicious path/ref fuzz → rejected with 0 shell-execution surfaces | Negative | Critical | High | AC-F12-1 |
...
- The duplicate-UUID gate runs after `assertBranchAllowed` but before document discovery.
...
#### TC-INTEGRATION-009 - Malicious path/ref fuzz → rejected with 0 shell-execution surfaces
...
**Steps**:
1. For each malicious path/ref fuzz fixture:
   2. Call `git.readCommitted("HEAD", [maliciousPattern])`.
   3. Assert the call returns `err({ kind: "BadPath", ... })` or a safe error.
   4. Assert NO shell execution occurs (e.g., no files outside the repo are accessed).
   5. For malicious refs: call `git.headSha()` or `git.currentBranch()` with the malicious ref.
   6. Assert the call returns `err({ kind: "BadRef", ... })` or a safe error.
   7. Assert NO shell execution occurs.

**Expected Outcome**:
- Every malicious path/ref fixture is rejected with a safe error.
...
- This test validates TDR-0003's verification criterion for shell-injection safety.
...
- **Fixtures**: Hardcoded fixture objects in test files (no external data files).
  ...
  - `applyPlan` fixtures: mock target responses (create/update/no-op/move/conflict/forbidden), lock fixtures, config fixtures.
...
| AC-F2-1 (INV-SAFE-3: duplicate-UUID fatal before any write) | TC-UNIT-001 | Unit |
...
| AC-F10-1 (INV-SEC-1: no secrets in output paths) | TC-UNIT-005, TC-UNIT-006 | Unit |
