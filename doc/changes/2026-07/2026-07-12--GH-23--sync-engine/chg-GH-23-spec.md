...
1. **Pure `computePlan`** — the no-writes pipeline: branch gate → duplicate-UUID fatal gate → discover committed docs via a new `Repository` port → parse → render+hash each via `TargetSystem.renderBody` → resolve cross-page links → fetch remote state per binding → classify each → emit a reviewable `Plan` carrying `{ runId, operationId, entries[] }`. Fully unit-testable with mocked git/target ports; dry-run returns here.
...
- **G-1**: Deliver pure `computePlan` — the no-writes pipeline: branch gate → duplicate-UUID fatal gate → discover → parse → render+hash → link-resolve → fetch remote → classify → emit a reviewable `Plan` (F-1).
...
- Integration tests: `applyPlan` against a `Bun.serve` mock target (create/update/no-op/move/conflict/forbidden); crash → replay; lock+property update; idempotency (0 writes on 2nd push).
...
| DM-4 | Journal entry (NEW, app/journal) | `{ ts: string; op: "create" \| "update" \| "move"; pageId: string; uuid: string; outcome: "success" \| "failed" }`. One JSON line per mutation in `.marksync/journal/<run-id>.jsonl`. |
