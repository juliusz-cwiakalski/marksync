...
| AC-2 | **Given** the Mermaid render pipeline (GH-69) attempts to upload an SVG attachment, **when** the upload is processed, **then** the schema-validation error no longer fires and the upload succeeds. **Traceability bridge to ticket AC #2 ("diagram appears as image on Confluence"):** the GH-69 render pipeline (`src/domain/mermaid/transform.ts` → Kroki SVG → `marksync-mermaid-<hash>.svg` → `uploadAttachment` → `<ac:image><ri:attachment>`) is merged and correct; this change fixes the ONLY broken link in that chain (the upload response parsing). Once `upload()` returns `ok(ref)` instead of `RemoteUnreachable`, the attachment is stored on the page and the `<ac:image>` reference resolves — so end-to-end rendering is restored. E2E live-sandbox verification (NG-5) is deferred because the fix is entirely in the response-parsing mapper (AC-1 unit-test coverage proves the contract); a manual smoke test (publish a page with a `render`-policy mermaid fence to a sandbox space) confirms the diagram appears as an image. | F-1, G-1 |
