...
| duplicate detection scale (non-functional — NFR-11) | `detectDuplicateUuids` is **O(n) by construction** (single-pass `Map<uuid, path[]>`); a **coarse smoke** over a ≤500-doc corpus **completes without error**. A strict ms-p95 budget is intentionally NOT asserted (CI-flaky across runners). |
...
- **F-2 (`DocumentId` value object).** `DocumentId` is a branded type `string & { __brand: "DocumentId" }` — a plain `string` is a compile error where a `DocumentId` is required. `parseDocumentId(s): Result<DocumentId, DocumentIdError>` (a narrow domain-local error type — NOT a `MarkSyncError` arm; see DEC-6) validates a candidate string with `isUuidV7` and returns the branded `DocumentId` or a typed error. `DocumentId` is the Ubiquitous-Language term for the Document Identity value (the UUID itself); it is the canonical identifier every downstream consumer (`PageBinding.uuid`, the lock, drift, sync) declares. The aggregate root is Document Identity; its logic lives in `src/domain/identity/` (UL binding rule 4).
...
| DM-6 | `DocumentIdError` (new, domain-local — NOT a `MarkSyncError` arm) | A narrow domain-local error type returned by `parseDocumentId` on an invalid candidate (DEC-6) — e.g. `{ kind: "InvalidDocumentId"; value: string }`. Lives alongside `DocumentId` in `src/domain/identity/document-id.ts`. It does NOT reach the application-tier mapper / `CODE_TO_EXIT`; recovery is caller-local (the caller decides whether to skip, warn, or halt). |

> Note: `Result<T, E>` and the `DuplicateUuid` arm already exist (GH-14/GH-16). This story adds **no** new `MarkSyncError` union arm and **no** new presentation-layer code/exit mapping — it is the first consumer of an arm landed speculatively. The net-new domain types are `DocumentId` (DM-1), `DocumentIdError` (DM-6 — a narrow domain-local type, NOT a `MarkSyncError` arm; DEC-6), and `PageBinding` (DM-3).
...
| NFR-11 | duplicate detection scale (demoted from hard AC-F4-3 to NFR — DoR iter-1 / PM-RECON-1 Decision D) | `detectDuplicateUuids` is **O(n) by construction** (single-pass `Map<uuid, path[]>`); a **coarse smoke** over a ≤500-doc corpus **completes without error**. A strict ms-p95 budget is intentionally NOT asserted (CI-flaky across runners); complexity is verified by construction, not wall-clock. |
...
|----|----------|---------|--------|
| OQ-1 | Should `assertUuidV7` throw or return `Result` on a malformed input? | typescript.md's "assertXxx" naming convention implies a throw/narrow; `parseDocumentId` already provides the `Result` path. | Resolved for delivery: `assertUuidV7` is an assertion-style narrowing guard (`asserts s is DocumentId`) used at internal boundaries where a non-v7 is a programmer error; `parseDocumentId` is the `Result`-returning path for untrusted input (formalized as DEC-6 — `DocumentIdError`, not a `MarkSyncError` arm). (Specification detail — no `@decision-advisor` escalation.) |
...
| DEC-6 | **`parseDocumentId` returns `Result<DocumentId, DocumentIdError>` where `DocumentIdError` is a NARROW domain-local type, NOT a new `MarkSyncError` union arm** (resolves the DoR iter-1 Finding 1 contradiction with DM-5/§16/§21). `DocumentIdError` lives alongside `DocumentId` in `src/domain/identity/document-id.ts` and is not re-exported into `MarkSyncError`. The error-handling rule is decisive: a new `MarkSyncError` union arm is added ONLY when its recovery action differs from all existing kinds — and no existing `MarkSyncError` kind fits an invalid document-id (it is not a `DuplicateUuid`, which is the only identity-class arm). Two channels, by intent: `assertUuidV7(s)` is the "should never happen" channel — it THROWS on invariant violation for internal/trusted paths; `parseDocumentId` is the expected-failure channel returning `Result` for untrusted input. | Keeps the `MarkSyncError` union and every exhaustive site (`assertNeverMarkSyncError`, the application-tier mapper, `CODE_TO_EXIT` → invariant class 50) UNCHANGED — consistent with DM-5, §8.5, §16, §21 ("no new `MarkSyncError` kind"). Avoids exhaustiveness churn in the union/mapper/presentation layers: the intake-verified `DuplicateUuid` arm is the only identity error that reaches the union, consumed unchanged; invalid-doc-id failures stay domain-local. (The plan tracks the equivalent decision in its own DEC-/PD-# namespace to avoid ID collision — see PM-RECON-1 Decision E.) | 2026-07-09 |

## 16. AFFECTED COMPONENTS (HIGH-LEVEL)

| Component | Impact |
|-----------|--------|
| Identity module (`src/domain/identity/`) | New — `generateUuidV7`/`isUuidV7`/`assertUuidV7`, `DocumentId`/`DocumentIdError`/`parseDocumentId`, `injectUuid`/`readUuid`, `detectDuplicateUuids` (UL binding rule 4 residence) |
...
| AC-F2-1 | **Given** a plain `string` is passed where a `DocumentId` is required, **when** the code is type-checked, **then** it is a compile error (verified by a `@ts-expect-error` test); and `parseDocumentId` of a valid v7 returns a `DocumentId`, of an invalid string returns `err(DocumentIdError)` (a narrow domain-local type — NOT a `MarkSyncError` arm; DEC-6, DM-6). | F-2, DM-1, DM-6, NFR-10 | (DocumentId branding) |
...
| (duplicate detection scale — derived from the O(n) approach; non-functional) | — (demoted from AC-F4-3 to NFR-11) | F-4, NFR-11 |
...
| 1.1 | 2026-07-09 | spec-writer (GH-18 DoR iter-1 reconciliation) | DoR iter-1 reconciliation (PM-RECON-1). **Finding 1 fix:** `parseDocumentId` signature changed from `Result<DocumentId, MarkSyncError>` → `Result<DocumentId, DocumentIdError>` — a narrow domain-local type, NOT a `MarkSyncError` arm (resolves the internal contradiction with DM-5/§16/§21). Added **DEC-6** (rationale: no existing arm fits an invalid doc-id; the error-handling rule forbids a new union arm unless recovery differs) and **DM-6** (`DocumentIdError`); updated F-2, the DM-table Note, §16 identity-module row, AC-F2-1, OQ-1, and the Authoring Guidelines accordingly. **Finding 5 (Decision D) fix:** demoted the scale requirement from hard **AC-F4-3** to **NFR-11**; relaxed the assertion from a strict ≤5 ms p95 (CI-flaky) to **O(n) by construction + a 500-doc coarse smoke (completes without error)**; updated §4.1, Appendix A, and the validation checklist. **Decision B** (first-collision-only) unchanged; **Decision E** (spec keeps DEC-#, plan renames its own to PD-#) noted in DEC-6. |
...
- **`parseDocumentId` error type (DEC-6):** `parseDocumentId` returns `Result<DocumentId, DocumentIdError>`, where `DocumentIdError` is a narrow domain-local type — NOT a `MarkSyncError` arm. This reconciles DoR iter-1 Finding 1: the original `Result<DocumentId, MarkSyncError>` signature was internally unfulfillable (no existing arm fits an invalid doc-id, and the error-handling rule forbids adding a union arm unless recovery differs from all existing kinds). The `DuplicateUuid` arm remains the ONLY identity error that reaches the union (consumed unchanged); invalid-doc-id failures stay domain-local. The plan tracks the equivalent decision in its own DEC-/PD-# namespace (PM-RECON-1 Decision E).
...
- **First-producer framing:** the `DuplicateUuid` error arm, its application-tier mapper case, and its `CODE_TO_EXIT` mapping already exist (landed speculatively in GH-14/GH-16). This story is the first **producer**; it adds no new `MarkSyncError` kind and no new presentation surface — reflected in DM-5, §8.5, §16, and §21. (The narrow domain-local `DocumentIdError` from DEC-6 is NOT a `MarkSyncError` arm and does not reach the union/mapper/exit pipeline.)
...
- [x] NFRs include measurable values (v7 regex + sort order; 0 bytes diff; O(n) single-pass + 500-doc smoke; ≤20 transitive deps; `bun run check` exit 0; compile-error branding)
