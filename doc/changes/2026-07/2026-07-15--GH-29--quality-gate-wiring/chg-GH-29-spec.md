...
- **G-3**: The BDD step definitions honor the over-mocking guardrail — they mock only the adapter ports (`TargetSystem` via `FakeTarget` and `Repository` via `FakeRepository`), never the domain logic (state classifier, hierarchy planner, push flow).
...
| Adapter ports mocked by BDD steps | 2 (`TargetSystem` + `Repository` — over-mocking guardrail; 0 domain modules mocked) |
...
Flow 2: BDD scenario honoring the over-mocking guardrail (INV-SAFE-2)
  ...
  (the state classifier + push flow are the REAL modules; only adapter ports — TargetSystem and Repository — are mocked)
...
| NFR-MAINT-1 | Over-mocking guardrail honored | 0 BDD steps mock the domain logic (state classifier, hierarchy planner, push flow); only the adapter ports are mocked (`TargetSystem` via `FakeTarget`, `Repository` via `FakeRepository`) |
...
| DEC-4 | **Mock only the adapter ports for the BDD scenarios (`TargetSystem` via `FakeTarget`, `Repository` via `FakeRepository`); the domain logic stays real.** | The over-mocking guardrail (TDR-0004 §"Test-design guardrail"; `testing-strategy.md` §"AI-agent over-mocking guardrail") forbids validating lifecycle invariants through mocks alone — the state classifier, hierarchy planner, and push flow (`computePlan`/`applyPlan`) must be real. The permitted mock seams are the adapter ports: `FakeTarget` (the `TargetSystem` port) and `FakeRepository` (the `Repository` Git port, for deterministic fixtures) — matching the integration-tier shape that already proves this. | 2026-07-15 |
...
| AC-F2-4 | **Given** a secret sentinel injected into the **document content** (Markdown source) of a managed doc — not into config/provenance (credentials themselves never enter `computePlan`/`applyPlan`; they live in the mocked `ConfluenceTarget`/`ConfluenceClient` adapter behind the `TargetSystem` port), **when** the real `computePlan` + `applyPlan` run and the plan, apply journal, lock, diagnostics, `version.message`, and cache are inspected, **then** the sentinel does not appear in any output path — INV-SEC-1 (the renderer/plan never echoes raw source secrets into these paths; mirrors `tests/integration/app/secrets-safety-integration.test.ts` TC-INTEGRATION-011). | F-2, G-3 |
| AC-F2-5 | **Given** any BDD scenario, **when** its step definitions are reviewed, **then** only the adapter ports are mocked (`TargetSystem` via `FakeTarget`, `Repository` via `FakeRepository`) — the domain logic (state classifier, hierarchy planner, push flow) is real (over-mocking guardrail; NFR-MAINT-1). | F-2, G-3, NFR-MAINT-1 |
...
| Over-mocking guardrail | TDR-0004 / `testing-strategy.md` rule: lifecycle invariants must be validated through integration/E2E paths, never mocks of the domain logic (state classifier, hierarchy planner, push flow); only the adapter ports (`TargetSystem`, `Repository`) may be mocked. |
| `TargetSystem` port | The ports-and-adapters boundary the Confluence adapter implements; one of the two adapter seams (`TargetSystem`, `Repository`) the BDD scenarios are permitted to mock — the domain logic must stay real. |
...
| 1.1 | 2026-07-15 | Change Spec Writer | DoR iter-1 fixes: DEC-4 adapter-port wording (F2); INV-SEC-1 content-injection injection point (F1) |
