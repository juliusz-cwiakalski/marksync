...
| 8 — CI unguard (OPEN-Q9) | DONE | 2026-07-07 | 2026-07-07 | 7bb384a | All `continue-on-error: true` + `|| true` removed (install/lint/typecheck/test/test:bdd/osv-scanner/license-audit). Bun pinned `1.2.23` in fast-loop matrix (NFR-6 narrowed: dep-audit floats). `fetch-depth: 0` on checkout so `commitlint --from origin/main` resolves. AC-F4-3 PASS: CI commit-lint step (`bunx commitlint --from origin/main --to HEAD`) added to fast-loop (TDR-0008 C-2 authoritative half). `check:boundaries` CI step added (TDR-0006). **OQ-2 RESOLVED:** osv-scanner V2 uses `scan -L` (the hyphenated `--lock-file` was not a valid V2 flag); license-audit now blocking. Boundary negative-test re-verified post-unguard (AC-F3-2 holds). |
| 9 — finalize + README | DONE | 2026-07-07 | 2026-07-07 | d08a057 | README "Current status" → MS-0002 in progress; "Getting started" dev-loop section (`bun install` / `bun run check` / `bun run src/cli/index.ts`). Final `bun run check` green (lint + format:check + typecheck + 4 tests + 0 boundary violations). No version bump (`0.0.0`, version_impact: none). Spec reconciliation handoff recorded for doc-syncer (phase 7): dev-environment.md script table + typescript.md 8→12 MarkSyncError drift. |
