...
- **G-5**: Deliver the `AttachmentService` (v1-only) — multipart upload + the 400-duplicate idempotency signal (F-5).
...
| attachment 400-dup idempotency | uploading a duplicate filename (identical bytes) → the adapter returns an "already exists" result (**not** an error); changed bytes → new hash → new filename → fresh create (hash-naming makes `/data` update unnecessary) |
...
| F-5 | `AttachmentService` (v1-only) | Multipart upload + the 400-duplicate-filename **idempotency signal** (not an error; spike H4). Hash-naming makes `/data` update unnecessary (changed bytes → new filename → fresh create). Powers idempotent rerun (NFR-PERF-4). |
...
Flow 5 — Idempotent attachment (400-dup = skip signal; spike H4):
  ...
  changed bytes → new hash → new filename → fresh create (hash-naming; `/data` update unnecessary).
...
- `AttachmentService` (v1-only) — multipart upload, 400-dup idempotency, `attachmentExists`, `listAttachments` (F-5). `/data` update removed as dead code (hash-naming makes it unnecessary).
...
| NFR-4 | attachment 400-dup idempotency (NFR-PERF-4) | duplicate filename (identical bytes) → "already exists" result (**not** an error); idempotent rerun performs 0 writes. Changed bytes → new hash → new filename → fresh create (hash-naming makes `/data` update unnecessary — removed as dead code) |
...
| `AttachmentService` (`src/infra/confluence/attachments.ts`) | New — v1 multipart, 400-dup idempotency (`/data` update removed: hash-naming makes it unnecessary) |
...
| AC-F5-1 | **Given** an upload of a duplicate filename (identical bytes, same hash), **when** the create endpoint returns 400 `"Cannot add a new attachment with the same file name"`, **then** the adapter returns an "already exists" result (**not** an error). Changed bytes produce a new hash → new filename → a fresh create (never a same-name dup), so the `/data` update path is unnecessary under hash-naming and was removed as dead code (documented in `attachments.ts`; review iter-1 finding 1, option c). | F-5, NFR-4 | AC4 (H4) |
