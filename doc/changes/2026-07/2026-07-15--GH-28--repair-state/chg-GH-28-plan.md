...
- [x] **2.6** **Dry-run guard (NFR-OBS-5):** when `opts.dryRun === true`, do NOT call `saveLock`, `computePlan`, or `applyPlan`. The report reflects only the planned diagnosis; `writes === 0`; the committed lock is untouched (AC-F4-1, TC-REPAIR-003/010). `interruptedRunDetected` is still reported so the plan shows what `--apply` would do.
