...
| **Page Binding** | The durable mapping between a Document Identity and a target-system page. Records: page ID, parent page ID, last-known remote version, content hashes, and the shared-base snapshot. Optional provenance fields `sourceBranch`, `commitCount`, `trimMarker` (GH-27; backward-compatible reading of pre-GH-27 locks). Code construct: the `PageBinding` interface (`src/domain/binding/page-binding.ts`) — landed as a type with identity-binding semantics (GH-18); first-persisted into the committed lock by GH-19 (`src/app/lock.ts`); the lock schema (`lock-schema.json`) mirrors the optional fields. | Entity (within Document Identity) | 1:1 → Target Page; 1:1 → Shared Base |
...
| **Provenance** | The source-path + Git-revision + last-sync metadata. Appears in three channels: (1) `version.message` on each Confluence page version (ADR-0010 squash summary, `formatVersionMessage`/`formatVersionMessageWithMeta`); (2) a human-visible Storage XHTML `{info}` macro panel at the page footer (`buildProvenancePanel`, carrying the `marksync:provenance-panel` marker; gated by `config.provenance.visiblePanel`); (3) the machine `marksync.metadata` content property (14-field JSON, ADR-0010 privacy: `commitCount` + `trimMarker` only, never subjects). `classifyVersion` distinguishes MarkSync-authored versions (`marksync git` prefix) from direct edits. Delivered GH-21 (version message) + GH-27 (panel + property + classifier). | Value object | attached to → Target Page |
...
| **Plan** | A deterministic, reviewable description of what MarkSync will do: a list of intended mutations with their sync states and hashes. Produced before any write; available via dry-run. Realized as the `Plan` type `{ runId; operationId; entries: PlanEntry[]; provenance; warnings; visiblePanel }` emitted by `computePlan` (`src/app/push-flow.ts`, GH-23; `visiblePanel` from `config.provenance.visiblePanel` — GH-27). A `PlanEntry` carries `{ uuid; sourcePath; state: SyncState \| "NEW"; action: PlanAction; hashes: ContentHash; renderedBody }`, where `PlanAction` extends the GH-22 `Action` (`NoOp`/`Update`/`Block`/`Skip`) with an app-tier `Create` arm for unbound documents (the GH-22 classifier is bound-documents-only — there is no `base` to classify a new doc against). | Value object | produced by → Planner / computePlan |
...
| **Mermaid Transform** | Domain service that renders `language-mermaid` fences to SVG and rewrites them into `<img>` attachment references. Realized as the pure-ish `transform(hast, config, renderer, sourcePath?)` (`src/domain/mermaid/transform.ts`, GH-69): a no-op unless `render.mermaid.policy === "render"`; otherwise walks HAST for `pre>code.language-mermaid`, renders each via the injected `Renderer` (forwarding `MermaidRenderConfig`, GH-76), receives a content-hashed `Artifact` (the adapter normalizes the SVG before hashing for deterministic identity, GH-76), dedups by source within a doc, and replaces the `pre` with an `img` node (→ `imageMacro` emits `<ac:image><ri:attachment>`). On `RemoteUnreachable` keeps the `pre` (falls back to the code block) and collects a warning (ADR-0002 C-2). Upload/dedup reuse the GH-26 asset pipeline (`uploadAssets`/`attachmentExists`). | Domain service | uses → Renderer; used by → Push Executor |
...
| **Renderer** | The domain-owned port (`interface Renderer`, `src/domain/mermaid/port.ts`) abstracting Mermaid diagram rendering. Declares `render(source, config): Promise<Result<Artifact, MarkSyncError>>` — the `config` carries `MermaidRenderConfig` so the adapter applies render options (GH-76). Active only under `render.mermaid.policy === "render"`; the default `code` policy never calls it. Infrastructure implements it via the Kroki HTTP adapter (`KrokiClient`, `src/infra/mermaid/kroki.ts`, GH-69 — ADR-0002 rung 6), which forwards config as diagram options and normalizes the SVG before hashing (GH-76); the in-process rung-1 adapter is deferred to MS-0003+. | Domain service (port) | implemented by → KrokiClient; used by → Mermaid Transform |

### Infrastructure Services

| Term | Meaning | Type | Relationships |
|---|---|---|---|
| **Push Executor** | The application-tier use-case orchestration that performs ordered safe writes via the `TargetSystem` port, journals each mutation, and handles optimistic concurrency (409 surfaced as drift; on `Conflict`, re-fetch + re-classify ONCE then reapply-or-block — max 1 re-fetch + 1 reapply, no loop). Realized as `computePlan` (pure, no-writes dry-run) + `applyPlan` (parent-first, per-document isolation, journal-before-lock, provenance via `formatVersionMessage`/`formatVersionMessageWithMeta` + visible panel injection via `appendProvenancePanel` + full 14-field `marksync.metadata` enrichment via `bindingToProperty` + direct-edit classification via `classifyVersion`) at `src/app/push-flow.ts` (GH-23; concurrency gates wired in GH-24; provenance panel + property + classifier — GH-27). Port construction is wired through the app-tier factory `createRepository`/`createTarget` (`src/app/ports.ts`). (Listed here because it owns the journal/409 contract the domain depends on; the orchestration is application-tier, while the primitives it delegates to — `writeAtomic`, the shell-git adapter — remain infrastructure.) | Application service | consumes → Plan; produces → Page Published, Drift Detected |
...
| **MetadataProperty** | The remote `marksync.metadata` content property (system spec §9.3): `{ schemaVersion, projectId, targetId, documentId, sourcePath, sourceCommit, sourceContentHash, renderedBodyHash, toolVersion, synchronizedAt, operationId, sourceBranch, commitCount, trimMarker }` — 14 fields. The last three (`sourceBranch`, `commitCount`, `trimMarker`) are optional for backward-compatible reading of pre-GH-27 properties. Per ADR-0010 privacy, the property stores `commitCount` + `trimMarker` only — never commit subjects (subjects appear only in `version.message`). Written by `bindingToProperty` (`src/app/push-flow.ts`, GH-27); caller-supplied input to the cross-check; fetched by the Confluence adapter. | Value object | input to → Content-Property Cross-Check |
...
| **Artifact** | `{ bytes; mime; hash; kind? }` — a binary artifact to upload (rendered Mermaid SVG, resolved asset). `hash` (full sha256) is the dedup key; optional `kind: "asset" \| "mermaid"` gates the filename prefix (`marksync-asset-` vs `marksync-mermaid-`). | Value object | input to → TargetSystem.uploadAttachment |
...
| **Mermaid Rendering** | Conformist (service) | The `Renderer` port (`src/domain/mermaid/port.ts`) is implemented by the `KrokiClient` HTTP adapter (`src/infra/mermaid/kroki.ts`); the domain mermaid transform (`src/domain/mermaid/transform.ts`) orchestrates render→upload→reference and dedup. **MS-0002 ships the `code` policy as the default** (rung 7 — mermaid fences preserved as code macros, GH-25) **and the opt-in `render` policy** via public Kroki (rung 6 — SVG rendered remotely, content-hashed, attached, with a one-time privacy warning; GH-69). The in-process rung-1 renderer (official `mermaid` + headless DOM) is the design target, deferred to MS-0003+ pending a faithful-render path (GH-11 H4 FAIL / CEO-DEC-1). |
