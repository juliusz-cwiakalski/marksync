...
links:
  ...
  related_changes: [GH-15, GH-17, GH-18, GH-19, GH-20, GH-21, GH-22, GH-23, GH-24, GH-26, GH-27, GH-74]
...
| **Mermaid Transform** | Domain service that renders `language-mermaid` fences to SVG and rewrites them into `<img>` attachment references. Realized as the pure-ish `transform(hast, config, renderer, sourcePath?)` (`src/domain/mermaid/transform.ts`, GH-69): a no-op unless `render.mermaid.policy === "render"`; otherwise walks HAST for `pre>code.language-mermaid`, renders each via the injected `Renderer`, content-hashes the SVG (full sha256), dedups by source within a doc, and replaces the `pre` with an `img` node (→ `imageMacro` emits `<ac:image><ri:attachment>`). On `RemoteUnreachable` keeps the `pre` (falls back to the code block) and collects a warning (ADR-0002 C-2). Upload/dedup reuse the GH-26 asset pipeline (`uploadAssets`/`attachmentExists`). | Domain service | uses → Renderer; used by → Push Executor |
