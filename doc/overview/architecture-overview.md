...
links:
  ...
  related_changes: [GH-18, GH-20, GH-21, GH-22, GH-23, GH-24, GH-26, GH-27, GH-63, GH-64, GH-66, GH-69, GH-76]
...
| Mermaid renderer (Kroki) | MarkSync binary | infrastructure | `KrokiClient` (`src/infra/mermaid/kroki.ts`) implements the `Renderer` port: `POST https://kroki.io/mermaid/svg` with the diagram source as `text/plain` and Mermaid config as diagram-options query params (`deterministic-ids`, `html-labels`; `securityLevel` not passed — blocked by Kroki, enforced as `strict` by default), 30 s `AbortController` timeout, HTTP 4xx/5xx + network/timeout → `RemoteUnreachable`; on 200 normalizes the SVG via `normalizeSvg` (`src/domain/mermaid/normalize.ts`, §3.3 rules) before computing full sha256, so `Artifact.bytes` is the normalized SVG and `hash = sha256(normalizedSVG)` (deterministic digests across syncs) and returns `Artifact { bytes, mime: "image/svg+xml", hash, kind: "mermaid" }` (ADR-0002 rung 6) *(delivered — GH-69; config passthrough + normalized hashing — GH-76)* |
