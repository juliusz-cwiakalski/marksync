...
//
// Centralized redaction layer (GH-16 D-4 / F-4 / INV-SEC-1 / NFR-SEC-2 /
// ADR-0011 C-5). Every byte of command output crosses this layer before write
// (the OutputService chokepoint applies it in Phase 4), so NO command can emit
// a raw secret to stdout/stderr — by construction, not by convention.
//
...
//
// Tier rule: presentation output. This module imports NO tier — not
// `#domain/*`, not `#infra/*`, not even a sibling output module. It is pure
// regex + string logic (DEC-1 / dep-cruiser `presentation-may-not-import-*`).
